...
static inline void fontConfig_initialize(fontConfig* config)
{
    ...
    config->userFontEntrys = (fontEntry*)0;
...
static inline s32 fontInit(fontConfig*config)
{
    ...
    extern s32 fontInitializeWithRevision(u64,fontConfig*);
...
typedef void  (*CellFontGetOutlineVertexCallback)(void*arg,int32_t contourN,int32_t vertexNumber,int32_t vertexAttr,float x,float y);
