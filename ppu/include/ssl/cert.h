...
int sslCertGetSerialNumber(const sslCert sslCert, const uint8_t **sboData, size_t *sboLength);
int sslCertGetPublicKey(const sslCert sslCert, const uint8_t **sboData, size_t *sboLength);
int sslCertGetRsaPublicKeyModulus(const sslCert sslCert, const uint8_t **sboData, size_t *sboLength);
int sslCertGetRsaPublicKeyExponent(const sslCert sslCert, const uint8_t **sboData, size_t *sboLength);
//int sslCertGetNotBefore(const sslCert sslCert, CellRtcTick *begin);
//int sslCertGetNotAfter(const sslCert sslCert, CellRtcTick *limit);
int sslCertGetSubjectName(const sslCert sslCert, const sslCertName *certName);
int sslCertGetIssuerName(const sslCert sslCert, const sslCertName *certName);

int sslCertGetNameEntryCount(const sslCertName certName, uint32_t *entryCount);
int sslCertGetNameEntryInfo(const sslCertName certName, uint32_t entryNum, const char **oidName, const uint8_t **value, size_t *valueLength, int32_t flag);
int sslCertGetMd5Fingerprint(const sslCert sslCert, const uint8_t *buf, uint32_t *plen);
